...
struct Verify: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        print("")
        print("retake: \(hashesByPreview.count - unstable.count) of \(hashesByPreview.count) previews reproducible over \(runs) runs")

        guard !unstable.isEmpty else {
            print("retake: rendering is reproducible; diffs from this repository can be trusted.")
            return
        }

        print("retake: \(unstable.count) preview\(unstable.count == 1 ? "" : "s") did not render identically:")
        for id in unstable {
        ...
        print("")
        print("""
        retake: these will show up as changes nobody made. Exclude them with \
