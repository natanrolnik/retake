...
struct HostProject {
    ...
    /// Local Swift packages to link, for a repository that does not use Tuist. The
    /// generated project then stands on its own rather than joining an existing graph.
    ...
    func write() throws {
        ...
        // With packages the scratch project is its own Tuist root, since the repository
        // has no config of its own to inherit.
        if !packages.isEmpty {
            try write(Self.standaloneConfig, to: "Tuist.swift")
            try FileManager.default.createDirectory(
                at: scratchDirectory.appendingPathComponent("Tuist"),
                withIntermediateDirectories: true
            )
            try write(packageManifest(), to: "Tuist/Package.swift")
        }
    }
    ...
    /// Declares the repository's local packages as dependencies of the scratch project.
    private func packageManifest() -> String {
        let dependencies = packages
            .map { #"        .package(path: "\#($0.directory.path)"),"# }
            .joined(separator: "\n")
        let productTypes = packages
            .flatMap(\.products)
            .map { #""\#($0)": .framework,"# }
            .joined(separator: "\n            ")
        return """
    ...
    private static let standaloneConfig = """
    // Generated by retake.
    ...
    private func manifest() -> String {
        ...
        switch host {
        case .synthesized(let linked) where !packages.isEmpty:
            // Packages are linked as external dependencies rather than by project path.
            hostTargetName = "RetakeHostApp"
            let links = packages.flatMap(\.products).map { product in
