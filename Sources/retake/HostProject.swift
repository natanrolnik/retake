...
struct HostProject {
    ...
    static let projectName = "RetakeHost"
    ...
    func write() throws {
        ...
        if case .synthesized = host {
            ...
            try write(Self.hostAppSource, to: "Sources/RetakeHostApp.swift")
        }
    ...
    private func manifest() -> String {
        let retakeSources = sources.retake.path
        ...
        var targets: [String] = [
            ...
            runtimeTarget(
                name: "RetakeCore",
                sources: ["\(retakeSources)/Sources/RetakeCore/**"],
                dependencies: []
            ),
            runtimeTarget(
                name: "RetakeRuntime",
                sources: ["\(retakeSources)/Sources/RetakeRuntime/**"],
                dependencies: [
            ...
            runtimeTarget(
                ...
                sources: ["\(retakeSources)/Sources/RetakeTestRuntime/**"],
                dependencies: [
                    #".target(name: "RetakeCore")"#,
                    #".target(name: "RetakeRuntime")"#,
                    #".target(name: "SnapshotPreviewsCore")"#,
                ],
        ...
        var testDependencies: [String] = [#".target(name: "RetakeTestRuntime")"#]

        switch host {
        case .synthesized(let linked):
            hostTargetName = "RetakeHostApp"
            ...
            targets.append("""
                ...
                """)
            testDependencies.append(#".target(name: "RetakeHostApp")"#)
        ...
        targets.append("""
                .target(
                    name: "RetakeHostTests",
                    destinations: [.iPhone],
                    product: .unitTests,
                    bundleId: "dev.retake.host.tests",
                    deploymentTargets: .iOS("\(deploymentTarget)"),
                    infoPlist: .default,
                    sources: ["Tests/**"],
                    dependencies: [
        ...
        return """
        // Generated by retake. Throwaway: this directory is deleted after rendering.
        //
        ...
        \(targets.joined(separator: "\n"))
            ...
            // what the project contains, and retake has to know the name up front.
            schemes: [
                .scheme(
                    name: "\(Self.schemeName)",
                    shared: true,
                    buildAction: .buildAction(targets: ["RetakeHostTests"]),
                    testAction: .targets(["RetakeHostTests"])
                ),
            ]
        )
    ...
    private func runtimeTarget(
        ...
        return """
                .target(
                    ...
                    destinations: [.iPhone],
                    product: .framework,
                    bundleId: "dev.retake.runtime.\(name.lowercased())",
                    deploymentTargets: .iOS("\(deploymentTarget)"),
                    infoPlist: .default,
                    sources: [\(sourceList)],
    ...
    // Generated by retake. An empty shell whose only job is to link the frameworks
    // under test, so their previews are reachable through runtime metadata.
    import SwiftUI

    @main
    struct RetakeHostApp: App {
        var body: some Scene {
    ...
    final class GeneratedPreviewSnapshots: RetakeSnapshotTests {}
