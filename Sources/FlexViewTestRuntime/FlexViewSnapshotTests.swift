...
/// final class PreviewSnapshots: FlexViewSnapshotTests {}
/// ```
///
/// The output directory comes from `FLEXVIEW_OUT` and is a **host** path, not a path
/// inside the simulator's container. `flexview render` forwards it as
/// `TEST_RUNNER_FLEXVIEW_OUT`, which xcodebuild passes to the test process.
open class FlexViewSnapshotTests: XCTestCase {
    ...
    override open class var defaultTestSuite: XCTestSuite {
        guard self != FlexViewSnapshotTests.self else {
            return XCTestSuite(name: "FlexViewSnapshotTests (abstract)")
        }
    ...
    public func testRenderPreviews() throws {
        ...
        do {
            ...
            // A missing FLEXVIEW_OUT means this ran as an ordinary test rather than
            // through `flexview render`, so there is nothing to do.
            print("flexview: skipping, \(error)")
            return
        }
        ...
        // `flexview diff` keeps them out of the removed bucket.
        for failure in manifest.failures {
            print("flexview: failed to render \(failure.previewID): \(failure.message)")
        }
