...
struct Comment: AsyncParsableCommand {
    ...
    var artifactName: String = "flexview-report"
    ...
    /// Updates the existing flexview comment when there is one, so pushes replace a
    ...
    private func upsert(markdown: String, pullRequest: Int) throws {
        ...
        let body = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("flexview-comment-\(pullRequest).md")
        ...
        if let identifier {
            ...
            print("flexview: updated comment \(identifier) on #\(pullRequest)")
        } else {
            _ = try Shell.runChecked("/usr/bin/env", [
                "gh", "api", "--method", "POST",
            ...
            print("flexview: posted a comment on #\(pullRequest)")
        }
    }
