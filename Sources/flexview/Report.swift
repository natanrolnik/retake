...
struct Report: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        print("flexview: wrote \(outputURL.path) (\(summary.headline))")
        if inlineImages {
            let size = (try? Data(contentsOf: outputURL).count) ?? 0
            print("flexview: \(ByteCountFormatter.string(fromByteCount: Int64(size), countStyle: .file)), self-contained")
        }
    }
}
